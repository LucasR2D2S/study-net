...
while (!shouldExit) 
{
    ...
    // If the player has consumed the food displayed.
    if (playerX == foodX && playerY == foodY) {
        ChangePlayer();
        // If the food consumed should freeze player movement.
        if (states[2] == player) {FreezePlayer();}
        if (IncreaseMovement()) {Move(3);}
...
bool IncreaseMovement() 
{
    if (player.Equals(states[1])) { return true;}
    else { return false;}
}

// If the food consumed should freeze player movement.
// If the food consumed should increase player movement.
